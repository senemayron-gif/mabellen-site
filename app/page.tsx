...
// Corrigido: Importando Instagram como InstagramIcon para evitar conflito de nomes
      ...
      {showMenu && (
        <div className="fixed inset-0 bg-black/95 z-[200] p-10 flex flex-col text-[#D4AF37] animate-in fade-in duration-300">
          ...
          <div className="flex flex-col gap-10 mt-20">
            ...
            <button onClick={() => { setIsAdmin(true); setShowMenu(false) }} className="flex items-center gap-6 text-lg text-gray-500 hover:text-gray-300 transition-colors uppercase tracking-widest text-left">
        ...
        <div className="fixed inset-0 bg-white z-[300] p-8 overflow-y-auto animate-in slide-in-from-bottom duration-500">
          <div className="max-w-md mx-auto space-y-8">
            <div className="flex justify-between items-center border-b pb-4">
              <b className="tracking-widest uppercase">Cadastrar Novo Produto</b>
              <button onClick={() => setIsAdmin(false)} className="p-2 bg-gray-100 rounded-full"><X/></button>
            </div>
            
            <div className="space-y-4">
              <p className="text-[10px] font-bold text-gray-400 tracking-widest uppercase">Foto Principal</p>
              ...
              <div className="flex gap-3 flex-wrap">
                {novoProd.fotos.map((f, i) => (
                  <img key={i} src={f} className="w-20 h-28 object-cover rounded-2xl shadow-md" alt="Preview" />
                ))}
              </div>
            </div>

            <div className="space-y-4">
              <input type="text" placeholder="Nome do Conjunto" className="w-full p-5 bg-gray-50 rounded-2xl outline-none" value={novoProd.nome} onChange={e=>setNovoProd({...novoProd, nome: e.target.value})} />
              <input type="text" placeholder="Preço (Ex: 159,90)" className="w-full p-5 bg-gray-50 rounded-2xl outline-none" value={novoProd.preco} onChange={e=>setNovoProd({...novoProd, preco: e.target.value})} />
            ...
            <button onClick={salvarProdutoCompleto} className="w-full bg-black text-[#D4AF37] py-6 rounded-3xl font-black uppercase tracking-[0.3em] shadow-xl">
              Publicar no Site
            </button>
          </div>
        </div>
          ...
          <div className="w-full max-w-md bg-white h-full p-10 flex flex-col shadow-2xl">
            ...
            {carrinho.length > 0 && (
              <div className="pt-8 border-t space-y-4">
                ...
                <button onClick={finalizarPedido} className="w-full bg-[#25D366] text-white py-6 rounded-3xl font-black flex items-center justify-center gap-3 uppercase text-[10px] tracking-[0.2em] shadow-lg hover:opacity-90">
